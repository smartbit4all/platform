...
public final class SB4ExecutionNode implements SB4Execution {
  ...
  public final List<SB4Function<?, ?>> getFunctions() {
    List<SB4Function<?, ?>> result = new ArrayList<>();
    for (SB4Execution sb4Execution : items) {
      if (sb4Execution instanceof SB4ExecutionFunction) {
        result.add(((SB4ExecutionFunction) sb4Execution).service);
      }
    }
    return result;
  }
