...
public class StorageFS extends ObjectStorageImpl {
  ...
  @Lazy
  private Optional<StorageTransactionManagerFS> transactionManager;
  ...
  protected Supplier<StorageObjectPhysicalLock> physicalLockSupplier(URI objectUri) {
    ...
    return () -> {
      StorageTransaction transaction =
          transactionManager.isPresent() ? transactionManager.get().getCurrentTransaction() : null;
  ...
  void addInvokeOnSucceedFunctions(StorageObject<?> object, ObjectVersion oldVersion,
    ...
    if (transactionManager.isPresent() && transactionManager.get().isInTransaction()) {
      transactionManager.get().addOnSucceed(object, event);
    } else {
