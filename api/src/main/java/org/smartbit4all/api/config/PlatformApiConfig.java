...
import org.smartbit4all.api.object.AccessControlApiImpl;
...
import org.smartbit4all.api.object.bean.ObjectContainerById;
...
import org.smartbit4all.api.object.extraction.DataExtractionApi;
import org.smartbit4all.api.object.extraction.DataExtractionApiImpl;
...
public class PlatformApiConfig {
  ...
  @Bean
  public ObjectDefinition<ObjectContainerById> objectDefinitionObjectContainerById() {
    return ObjectDefinitionApiImpl.constructDefinitionBase(ObjectContainerById.class)
        .idPath(ObjectContainerById.ID);
  }
  ...
  MDMDefinitionOption systemIntegrationPlatformMdmOption(LocaleSettingApi localeSettingApi) {
    ...
    }
    {
      MDMEntryDescriptor entry = new MDMEntryDescriptor()
          ...
          .displayNameList(new LangString().defaultValue("Database connections")
              .putValueByLocaleItem("hu", "Adatbázis kapcsolatok")
              .putValueByLocaleItem("en", "Database connections"))
          .displayNameForm(new LangString().defaultValue("Database connection")
              .putValueByLocaleItem("hu", "Adatbázis kapcsolat")
              .putValueByLocaleItem("en", "Database connection"))
          .order(200l)
          .typeQualifiedName(ServiceConnection.class.getName())
          .addTableColumnsItem(
              new MDMTableColumnDescriptor()
    ...
    }
    {
      MDMEntryDescriptor entry = new MDMEntryDescriptor()
          ...
          // FIXME: MdmEntrySetup cannot update if an entry has a unique constraint.
          // .addConstraintsItem(new MDMEntryConstraint()
          // .kind(KindEnum.UNIQUECASEINSENSITIVE)
  ...
  @Bean
  public AccessControlApi accessControlApi() {
    return new AccessControlApiImpl();
  }
