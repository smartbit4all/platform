...
public class PropertyRef<T> extends Property<T> {
  ...
  public PropertyRef(String name, List<Reference<?, ?>> joinPath, Property<T> referredProperty) {
    ...
    if (referredProperty instanceof PropertyObject) {
      referredProperty = (Property<T>) ((PropertyObject) referredProperty).getBasic();
    }
  ...
  public final PropertyOwned<T> getReferredOwnedProperty() {
    Property<T> prop;
    if (referredProperty instanceof PropertyObject) {
      prop = (Property<T>) ((PropertyObject) referredProperty).getBasic();
    } else {
      prop = referredProperty;
    }
    if (prop instanceof PropertyOwned) {
      return (PropertyOwned<T>) prop;
    } else if (prop instanceof PropertyRef) {
      return ((PropertyRef<T>) prop).getReferredOwnedProperty();
    } else {
