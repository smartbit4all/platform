...
import org.smartbit4all.ui.vaadin.layout.size.Top;
...
public class MultiSelectPopUp<T> extends CustomField<Set<T>> implements HasDataProvider<T> {
	...
	private FlexBoxLayout buttonBox;
	...
	private void initOuterComponents() {
		...
		buttonBox = new FlexBoxLayout(btnClear, icnInfo);
		buttonBox.setPosition(Position.ABSOLUTE);
		...
		String textEndGradientStyle = "--_lumo-text-field-overflow-mask-image: linear-gradient(to left, transparent 2.6em, #000 5.05em)";
	...
	private void initDialogComponents() {
		...
		dialog = new Dialog();
		FlexBoxLayout dialogLayout = new FlexBoxLayout();
		...
		FlexBoxLayout dialogButtonLayout = new FlexBoxLayout();
		dialogButtonLayout.setJustifyContentMode(JustifyContentMode.EVENLY);
		dialogButtonLayout.setMargin(Top.M);
	...
	private void setupComponents() {
		...
		/*
		 * When the filter field is cleared and the select all cb was checked, it
		 * remains checked after no matter if there are unchecked items in the whole
		 * list. To avoid this, we deselect and select one item causing an update on the
		 * checkbox ui.
		 */
		...
		/*
		 * because of a vaadin grid bug that causes js exceprions poping up on the ui
		 * the grid selection has to be manually cleared before detach (before closing the dialog)
		 * and reset on open...
		 * the bug is fixed, but not released in vaadin platform yet :(
...
//			return getPlaceHolder() + " - " + String.valueOf(selectionSize);
	...
	/**
	 * Returns the display components of the selected items. If only String values
	 * are displayed then it is wrapped in a {@link Text}.
	 */
	...
	protected Set<T> generateModelValue() {
		return new HashSet<T>(grid.getSelectedItems());
	}
