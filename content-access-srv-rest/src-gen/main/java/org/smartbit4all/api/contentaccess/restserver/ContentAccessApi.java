...
public interface ContentAccessApi {
  ...
  @ApiResponses({
      @ApiResponse(code = 200, message = "File with the given uuid.",
          response = org.springframework.core.io.Resource.class)
  })
  @RequestMapping(
  ...
  default ResponseEntity<org.springframework.core.io.Resource> download(
      @ApiParam(value = "", required = true) @PathVariable("uuid") UUID uuid) throws Exception {
  ...
  default ResponseEntity<Void> upload(
      @ApiParam(value = "") @Valid @RequestParam(value = "uuid", required = false) UUID uuid,
